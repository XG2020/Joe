<aside class="joe_aside">
    <section class="joe_aside__item author">
        ...
        <div class="user">
            <img width="75" height="75" class="avatar lazyload" src="<?php _getAvatarLazyload(); ?>" data-src="<?php $this->options->JAside_Author_Avatar ? $this->options->JAside_Author_Avatar() : _getAvatarByMail(_getAsideAuthorInfo($this, 'mail')) ?>" alt="博主头像" />
            <a class="link" href="<?php $this->options->JAside_Author_Link() ?>" target="_blank" rel="noopener noreferrer nofollow"><?php echo $this->options->JAside_Author_Nick ? $this->options->JAside_Author_Nick : _getAsideAuthorInfo($this, 'screenName'); ?></a>
    ...
    <?php endif; ?>
    <?php if ($this->options->JRanking && $this->options->JRanking !== 'off') : ?>
        <?php
        ...
        <section class="joe_aside__item ranking">
            <div class="joe_aside__item-title">
                ...
                <span class="text"><?php echo $rankingStr[0] ?></span>
                <span class="line"></span>
            </div>
            <ul class="list">
                <?php
                if ($rankingStr[1] === "bilibili") {
                    if (isset($rankingStr[2]) && $rankingStr[2] === "video") {
                    ...
                    if (isset($res['code']) && $res['code'] === 0) {
                        for ($i = 0; $i < count($res['data']['list']); $i++) {
                            if ($i < 9) {
                                echo
                                    "<li title='" . htmlspecialchars($res['data']['list'][$i]['title']) . "'>
                                        <span>" . ($i + 1) . "</span>
                                        <a target='_blank' rel='noopener noreferrer nofollow' href='" . $res['data']['list'][$i]['short_link_v2'] . "'>" . htmlspecialchars($res['data']['list'][$i]['title']) . "</a>
                                    </li>";
                            }
                        }
                    } else {
                    ...
                    if (isset($res['code']) && $res['code'] === 0) {
                        for ($i = 0; $i < count($res['result']['list']); $i++) {
                            if ($i < 9) {
                                echo
                                    "<li title='" . htmlspecialchars($res['result']['list'][$i]['title']) . "'>
                                        <span>" . ($i + 1) . "</span>
                                        <a target='_blank' rel='noopener noreferrer nofollow' href='" . $res['result']['list'][$i]['url'] . "'>" . htmlspecialchars($res['result']['list'][$i]['title']) . "</a>
                                    </li>";
                            }
                        }
                    } else {
